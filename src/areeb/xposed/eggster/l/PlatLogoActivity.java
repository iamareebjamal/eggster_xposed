	...
	@Override
	  public void onCreate(Bundle savedInstanceState) {
	    ...
	    setContentView(Cont);		  
			...
			if (sui.equals(getString(R.string.pref_translucent))) label.bottomMargin = 100;
		  Cont.addView(rct2);  Cont.addView(rct1);  Cont.addView(build, label);
		  rct1.setLayoutParams(pos1);		 rct2.setLayoutParams(pos2);
		  

		  new CountDownTimer (Integer.MAX_VALUE, 1000) {
			     public void onTick(long millisUntilFinished) {
						LayoutParams newpos1 = new LayoutParams(r.nextInt(1000), r.nextInt(1000)),
								 	 newpos2 = new LayoutParams(r.nextInt(1000), r.nextInt(1000));
				...
				Cont.setOnLongClickListener(new OnLongClickListener() {
					...
					public boolean onLongClick(View v) {
						if (Build.VERSION.SDK_INT > 10) {
							try {
								startActivity(new Intent(Intent.ACTION_MAIN).setFlags(Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS).setClassName("areeb.xposed.eggster", "areeb.xposed.eggster.kk.DessertCase"));
